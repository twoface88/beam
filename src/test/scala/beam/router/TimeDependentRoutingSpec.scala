...
class TimeDependentRoutingSpec
  ...
  "A time-dependent router" must {
    ...
    "give updated travel times for a given route" in {
      val leg = BeamLeg(3000,
                        BeamMode.CAR,
                        0,
                        BeamPath(Vector(143, 60, 58, 62, 80, 74, 68, 154),
                                 None,
    ...
    "take given link traversal times into account" in {
      router ! RoutingRequest(
        ...
        Vector(),
        Vector(
          StreetVehicle(Id.createVehicleId("car"),
    ...
    "give updated travel times for a given route after travel times were updated" in {
      router ! UpdateTravelTime((_: Link, _: Double, _: Person, _: Vehicle) =>
        1000) // Every link takes 1000 sec to traverse.
      val leg = BeamLeg(28800,
                        BeamMode.WALK,
                        0,
                        BeamPath(Vector(1, 2, 3, 4),
                                 None,
