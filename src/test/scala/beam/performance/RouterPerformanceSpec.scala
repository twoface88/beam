...
class RouterPerformanceSpec
  ...
  override def beforeAll(configMap: ConfigMap): Unit = {
    ...
    val networkCoordinator: NetworkCoordinator =
      new NetworkCoordinator(beamConfig, VehicleUtils.createVehiclesContainer())
...
//            response.itineraries.foreach(i => println(s"links#${i.beamLegs().map(_.travelPath.linkIds.size).sum}, time:${i.totalTravelTime}"))
  ...
  def testMatsim(routerAlgo: LeastCostPathCalculator) {
    ...
    runSet.foreach(n => {
      ...
      testSet.foreach({
        pare =>
          val path = routerAlgo.calcLeastCostPath(pare(0),
                                                  pare(1),
                                                  8.0 * 3600,
                                                  null,
                                                  null)
  ...
  def getMultiNodeDijkstra(): LeastCostPathCalculator = {
    ...
    val travelDisutility = new RandomizingTimeDistanceTravelDisutilityFactory(
      TransportMode.car,
      matsimConfig.planCalcScore).createTravelDisutility(travelTime)
  ...
  def getAStarEuclidean(): LeastCostPathCalculator = {
    ...
    new AStarEuclideanFactory().createPathCalculator(network,
  ...
  def getFastAStarEuclidean(): LeastCostPathCalculator = {
    val travelTimeCostCalculator = new FreespeedTravelTimeAndDisutility(
      new PlanCalcScoreConfigGroup)
    new FastAStarEuclideanFactory().createPathCalculator(
      network,
      travelTimeCostCalculator,
      travelTimeCostCalculator)
  }
  ...
  def getFastDijkstra(): LeastCostPathCalculator = {
    val travelTimeCostCalculator = new FreespeedTravelTimeAndDisutility(
      new PlanCalcScoreConfigGroup)
    new FastDijkstraFactory().createPathCalculator(network,
                                                   travelTimeCostCalculator,
                                                   travelTimeCostCalculator)
  }
  ...
  def buildRequest(transportNetwork: TransportNetwork,
                   fromFacility: Activity,
                   toFacility: Activity): ProfileRequest = {
