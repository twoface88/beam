...
import org.scalatest.{BeforeAndAfterAll, FunSpecLike, ParallelTestExecution}
...
class PersonAgentSpec
  ...
  val modeChoiceCalculator = new ModeChoiceCalculator {
    ...
    override def utilityOf(mode: Modes.BeamMode,
                           cost: Double,
                           time: Double,
                           numTransfers: Int): Double = 0.0
  }
  ...
  describe("A PersonAgent") {
    ...
    it("should know how to take a car trip when it's already in its plan") {
      ...
      val route = RouteUtils.createLinkNetworkRouteImpl(Id.createLinkId(1),
                                                        Array[Id[Link]](),
                                                        Id.createLinkId(2))
      ...
      personActor ! RoutingResponse(
        Vector(
          EmbodiedBeamTrip(
            Vector(
              EmbodiedBeamLeg(embodyRequest.leg.copy(duration = 1000),
                              Id.createVehicleId("body-dummyAgent"),
                              true,
                              None,
                              BigDecimal(0),
    ...
    it(
      ...
      val vehicleType = new VehicleTypeImpl(Id.create(1, classOf[VehicleType]))
      val bus = new BeamVehicle(new Powertrain(0.0),
                                new VehicleImpl(Id.createVehicleId("my_bus"),
                                                vehicleType),
                                None,
                                Car,
                                None,
                                None)
      val tram = new BeamVehicle(new Powertrain(0.0),
                                 new VehicleImpl(Id.createVehicleId("my_tram"),
                                                 vehicleType),
                                 None,
                                 Car,
                                 None,
      ...
      scheduler ! ScheduleTrigger(NotifyLegStartTrigger(28800, busLeg.beamLeg),
                                  personActor)
      scheduler ! ScheduleTrigger(NotifyLegEndTrigger(29400, busLeg.beamLeg),
                                  personActor)
      scheduler ! ScheduleTrigger(NotifyLegStartTrigger(29400, busLeg2.beamLeg),
                                  personActor)
      scheduler ! ScheduleTrigger(NotifyLegEndTrigger(30000, busLeg2.beamLeg),
                                  personActor)
      lastSender ! ReservationResponse(
        reservationRequestBus.requestId,
        Right(
          ReserveConfirmInfo(busLeg.beamLeg,
                             busLeg2.beamLeg,
      ...
      lastSender ! ReservationResponse(
        ...
        Right(
          ReserveConfirmInfo(tramLeg.beamLeg,
                             tramLeg.beamLeg,
                             reservationRequestBus.passengerVehiclePersonId)))
      scheduler ! ScheduleTrigger(NotifyLegStartTrigger(30000, tramLeg.beamLeg),
                                  personActor)
      scheduler ! ScheduleTrigger(NotifyLegEndTrigger(32000, tramLeg.beamLeg),
                                  personActor) // My tram is late!
      events.expectMsgType[PersonEntersVehicleEvent]
      events.expectMsgType[PersonLeavesVehicleEvent]
