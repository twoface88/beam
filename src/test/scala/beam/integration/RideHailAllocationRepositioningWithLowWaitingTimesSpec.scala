...
class RideHailAllocationRepositioningWithLowWaitingTimesSpec
    extends FlatSpec
    with BeamHelper
    with MockitoSugar
  ...
  it should "be able to run for 1 iteration without exceptions" in {
    val config = testConfig("test/input/beamville/beam.conf")
      ...
      .withValue(
        "beam.agentsim.agents.rideHailing.allocationManager",
        ConfigValueFactory.fromAnyRef("REPOSITIONING_LOW_WAITING_TIMES"))
      .withValue("beam.agentsim.agents.modalBehaviors.modeChoiceClass",
                 ConfigValueFactory.fromAnyRef("ModeChoiceRideHailIfAvailable"))
      .withValue(
        "beam.agentsim.agents.rideHailing.numDriversAsFractionOfPopulation",
